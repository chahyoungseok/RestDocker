...
public class KakaoOAuthUtils {
    ...
    public KakaoOAuthLoginInfoDto kakaoOAuthLogin(String authorizationCode) throws OpenApiException {
        OAuthTokenDto accessTokenInfo = getAccessToken(authorizationCode);
        return getAccountInfo(accessTokenInfo);
    }

    public boolean kakaoOAuthLogout(String oauthAccessToken, Long accountId) throws OpenApiException {
    ...
    private KakaoOAuthLoginInfoDto getAccountInfo(OAuthTokenDto accessTokenInfo) throws OpenApiException {
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_FORM_URLENCODED);
